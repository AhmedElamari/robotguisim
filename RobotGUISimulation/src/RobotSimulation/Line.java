...
public class Line {
	...
	/**
	 * calculate the gradient and offset of the line already ascertained that is not
	 * vertical
	 */
	...
	/**
	 * See if the line intersects with otherLine, return true if so in which case
	 * calcaulate in xyp the point of intersection
	 * 
	 * @param otherLine
	 * @return if true
	 */
	public boolean findintersection(Line otherLine) {
		...
		if (isVertical()) { // is vertical line
			if (otherLine.isVertical())
				isOne = false; // two vertical lines dont intersect
			else {
		...
		if (isOne)
			isOne = isOnLine(xy) && otherLine.isOnLine(xy);
	...
	public double distanceFrom(double x, double y) {
		...
		if (isOnLine(xy))
			...
			if (sdist2 < sdist)
				sdist = sdist2; // select shorter of two
		}
		return sdist;
	}
